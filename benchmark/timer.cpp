...
// Static declarations 
std::vector<std::string> Timer::s_labels;
std::vector<unsigned> Timer::s_labels_occurrencies;
std::vector<timelist_t> Timer::s_times;

Timer::Timer(std::string label_arg) {
    this->m_StartTimepoint = std::chrono::high_resolution_clock::now();

    this->label = label_arg;
    int _label_index = search_label(label_arg);
    
    // New label
    if(_label_index == -1) {
        this->label_index = s_labels.size();
        s_labels.push_back(label_arg);
        
        timelist_t times_measurements(1);
        times_measurements[0] = 0;
        s_times.push_back(times_measurements);

        s_labels_occurrencies.push_back(1);
    }

    // Found an alredy existing label
    else {
        this->label_index = _label_index;
        s_labels_occurrencies[_label_index] += 1;
    }
...
void Timer::Stop() {
    ...
    s_times.at(this->label_index).push_back(duration/1000);
/*
    double ms = duration * 0.001;
    std::cout << this->label << ": " << duration << "us (" << ms << "ms)\n";*/
}

void Timer::s_print_times() {
    std::cout << std::endl;
    for(unsigned count=0; count<s_labels.size(); count++) {
        std::cout << s_labels[count] << "(calls=" << s_labels_occurrencies[count] <<  "): "
        << s_compute_mean(s_times[count]) << " +- " << s_compute_conf_int(s_times[count]) << "us" << std::endl;
        /*
...
int Timer::search_label(std::string target_label) {
    for(unsigned count=0; count<s_labels.size(); count++) 
        if(s_labels[count] == target_label) return count;
    return -1;
}

long Timer::s_compute_mean(timelist_t times) {
    long sum = 0;
...
long Timer::s_compute_conf_int(timelist_t times) {
    times.erase(times.begin());
    long mean = s_compute_mean(times);

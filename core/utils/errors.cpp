...
std::string get_error_message(server_error_t error) {
    ...
    switch (error) {
        ...
        case PUBLICATION_ENCLAVE_ERROR:
        return std::string("Error inside enclave_publication_wrapper");
        break;

        case UNSEAL_CLIENT_KEY_ERROR:
        return std::string("Error unsealing user key");
        break;

        case UNSEAL_SERVER_KEY_ERROR:
        return std::string("Error unsealing server key");
        break;

        case RESULT_BUFFER_OVERFLOW_ERROR:
        return std::string("Insuficient buffer memory to compute result");
        break;
